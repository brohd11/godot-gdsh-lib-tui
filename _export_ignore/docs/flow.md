# A list → text entry → confirmation flow

Save the following as `res://commands/edit_list.gd` and load that directory into a
GDSh console. The example edits an in-memory list; it does not write files or register
any commands by itself.

Enter on the list pushes an editing screen. Enter in the text entry pushes a
confirmation. No returns to the entry with its text and caret intact; Yes returns
the edited value to the list. Escape in the entry cancels that edit, and Escape in
the list exits the command.

Each screen owns one component and explicitly gives it its available size. Navigation
is requested by signal handlers; results return through `resume()`. The router applies
these requests after the current update, so an Enter press cannot both open and
...
This example is exercised by `tests/gdsh_lib/fixtures/tui_flow.gd` and the router tests.
For several components on one screen, allocate their sizes in `set_size()`, join their
views in `view()`, and let the screen base manage Tab/Shift+Tab focus.
